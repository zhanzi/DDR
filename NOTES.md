...
#### 字体大小优化
- **分组标题**：`fontSize: '0.9rem'` (14.4px) + `fontWeight: 600`
- **菜单项**：`fontSize: '0.8125rem'` (13px) + `fontWeight: 500`
- **设计原则**：分组标题作为"标题"应该比菜单项更突出
- **移除全大写**：`textTransform: 'none'` 提升可读性
...
### 2024-12-20
- **新增了终端事件列表页面，支持全局事件搜索和筛选**：
  - 功能：创建了一个独立的终端事件列表页面，用户可以搜索和查看所有终端的事件信息
  - 实现内容：
    1. **后端API接口**：
       - 新增`GET /api/Terminals/events`接口，支持全局终端事件查询
       - 支持多种筛选条件：商户ID、终端唯一码、设备编号、事件类型、时间范围
       - 使用联接查询获取终端信息，避免导航属性
       - 实现权限控制：非系统管理员只能查看自己商户的事件
    2. **前端页面**：
       - 创建`TerminalEventsList.jsx`页面，路径为`/app/terminal-events`
       - 支持商户选择、终端唯一码、设备编号、事件类型、开始时间、结束时间筛选
       - 使用DateTimePicker支持精确的时间选择
       - 表格显示事件详情，包括事件时间、商户、终端信息、事件类型、严重性等
       - 支持分页显示和点击查看终端详情
    3. **路由和导航**：
       - 在路由配置中添加`/app/terminal-events`路径
       - 在侧边栏菜单中添加"终端事件"导航项，使用CpuIcon图标
       - 权限控制：仅SystemAdmin和MerchantAdmin角色可访问
    4. **数据结构优化**：
       - 扩展`TerminalEventDto`，添加`MachineID`和`DeviceNO`字段
       - 统一使用`MachineID`作为终端唯一码字段名，保持与数据模型一致
  - 影响：用户现在可以在一个页面中搜索和查看所有终端的事件信息，提高了事件管理的效率

- **优化了终端事件列表页面的时间筛选功能**：
  - 问题：原始实现只支持DateTimePicker，对于只需要按日期筛选的场景不够友好
  - 解决方案：
    1. **参考终端记录页面的设计**：添加精确时间切换开关
    2. **双模式时间选择**：
       - 默认模式：使用DatePicker只选择日期，适合大部分查询场景
       - 精确模式：使用DateTimePicker选择具体时间（小时分钟），适合精确查询
    3. **用户体验优化**：
       - 添加"精确时间选择（包含小时分钟）"开关
       - 使用24小时制格式（ampm={false}）
       - 统一的日期格式：yyyy/MM/dd 和 yyyy/MM/dd HH:mm
       - 当精确时间开关变化时自动重新搜索（如果已有时间选择）
    4. **本地化支持**：
       - 使用中文日期选择器本地化配置
       - 统一的日期格式显示
  - 影响：用户可以根据需要选择合适的时间筛选精度，提升了查询的灵活性和用户体验

- **优化了终端事件列表页面的商户信息显示**：
  - 问题：事件列表页面只显示商户ID，用户体验不够友好，需要参考终端记录页面的显示方式
  - 解决方案：
    1. **后端API优化**：
       - 修改`GET /api/Terminals/events`接口，添加商户表的联接查询
       - 扩展`TerminalEventDto`，添加`MerchantName`字段
       - 在查询中联接`Merchants`表获取商户名称信息
    2. **前端显示优化**：
       - 表格列标题从"商户ID"改为"商户"
       - 优先显示商户名称，如果有商户名称则显示名称
       - 使用Tooltip显示完整的商户ID信息
       - 商户名称过长时使用省略号显示，鼠标悬停查看完整信息
    3. **用户体验提升**：
       - 商户名称限制最大宽度150px，避免表格列过宽
       - 添加cursor: 'help'样式，提示用户可以查看详细信息
       - 保持与终端记录页面一致的显示风格
  - 影响：用户现在可以直观地看到商户名称，提升了事件列表的可读性和用户体验

- **优化了终端离线时长显示，提供更友好的时间格式**：
  - 问题：终端管理页面和终端详情页面的离线时长显示都是"离线XXXXX分钟"格式，对于长时间离线的终端显示不友好
  - 解决方案：
    1. **创建智能时长格式化函数**：
       - 根据离线时长自动选择合适的时间单位
       - 支持分钟、小时、天、月、年的显示格式
    ...
    2. **优化显示逻辑**：
       ...
       - 超过1年：显示"离线X年"
    3. **统一两个页面的实现**：
       - 终端列表页面：`TerminalList.jsx`中的`formatOfflineDuration`函数
       - 终端详情页面：`TerminalDetail.jsx`中的相同函数
       - 保持在线判断逻辑一致：使用`isWithinMinutes(status.lastActiveTime, 5)`
    4. **代码优化**：
       - 清理了终端详情页面中未使用的导入项
       - 确保两个页面的时间处理逻辑完全一致
  - 影响：用户现在可以看到更直观的离线时长信息，如"离线2天"、"离线3小时"等，提升了用户体验
...
- **创建了WebAdmin项目系统架构设计文档**：
  ...
  - 文档内容：
    1. **系统概述**：业务背景、系统目标、核心功能模块
    2. **架构设计**：总体架构、分层架构、服务化架构、数据流设计
    3. **技术选型**：后端技术栈、前端技术栈、基础设施技术栈及选型理由
    4. **模块设计**：模块划分、核心模块设计、模块间通信
    5. **数据架构**：数据库设计原则、核心数据模型、数据关系图、优化策略
    6. **接口设计**：API设计原则、API规范、核心API接口
    7. **安全架构**：安全设计原则、认证架构、授权架构、数据安全、安全监控
    8. **部署架构**：部署模式、容器化设计、高可用设计
    9. **性能设计**：性能目标、性能优化策略
    10. **扩展性设计**：水平扩展、垂直扩展、技术演进
  - 特点：
    - 专注于系统架构设计，不包含具体问题修复记录
    - 涵盖从业务需求到技术实现的完整架构设计
    - 包含详细的技术选型理由和架构决策
    - 提供清晰的模块划分和数据模型设计
    - 考虑了系统的可扩展性和技术演进路径
  - 文档长度：约1300行，专注于架构设计本身

- **修复了文件类型管理删除功能的用户体验问题**：
  - 问题现象：删除已使用的文件类型时，控制台有报错，错误提示是原始的弹出信息，且删除确认窗口没有自动关闭
  - 根本原因：
    1. 其他管理页面（MessageTypeList.jsx、FilePublishList.jsx）仍在使用`alert()`显示错误信息
    2. axios响应拦截器输出过多控制台日志，包括业务逻辑错误
    3. 文件类型管理页面的删除功能虽然已正确实现，但缺少成功提示
  - 解决方案：
    1. **统一错误处理方式**：
       - 将MessageTypeList.jsx和FilePublishList.jsx中的`alert()`改为`enqueueSnackbar`
       - 添加友好的错误信息解析，支持字符串和对象格式的错误响应
       - 确保删除确认对话框在错误时也能正确关闭
    2. **优化控制台日志输出**：
       - 修改axios响应拦截器，只在开发环境输出详细日志
       - 只记录真正的服务器错误（>=500），不记录业务逻辑错误（如400错误）
       - 恢复401错误的自动跳转到登录页面功能
    3. **完善成功提示**：
       - 为所有删除操作添加成功提示消息
       - 使用统一的Snackbar组件显示操作结果
  - 技术细节：
    - 在所有相关页面添加`useSnackbar` hook的导入和使用
    - 错误信息解析支持`error.response.data`的字符串和对象格式
    - 响应拦截器使用`import.meta.env.DEV`判断开发环境
  - 影响：现在删除操作有友好的成功/失败提示，控制台日志更清晰，用户体验大幅提升

- **实现了统一的前端错误处理机制**：
  - 问题现象：多个页面在后台API返回失败或异常时会导致前端页面崩溃，缺乏友好的错误提示
  - 根本原因：
    1. 错误处理不统一，有些页面只有`console.error`没有用户提示
    2. 错误信息解析不够健壮，无法处理各种格式的后端错误响应
    3. 缺少网络错误、权限错误等特殊情况的处理
  - 解决方案：
    1. **创建通用错误处理工具**：
       - 新增`utils/errorHandler.js`工具文件
       - 实现`parseErrorMessage`函数，统一解析各种错误格式
       - 支持HTTP状态码映射、网络错误检测、后端错误信息提取
       - 提供`withSnackbarErrorHandling`等包装器函数
    2. **统一错误提示方式**：
       - 优先使用`enqueueSnackbar`显示错误提示（左下角友好提示）
       - 保留部分页面的自定义`showMessage`函数（如TerminalList）
       - 为成功操作也添加友好提示
    3. **修复关键页面错误处理**：
       - FilePublish.jsx：修复发布失败时的错误处理和成功提示
       - FilePublishList.jsx：添加加载失败的错误提示
       - MessageTypeList.jsx：完善创建/更新/删除的错误处理
       - MessageSend.jsx：改进消息发送失败的错误解析
       - Dashboard页面：统一仪表盘数据加载失败的错误处理
       - TerminalList.jsx：简化错误处理逻辑，使用通用工具
       - TerminalDetail.jsx：改进终端信息加载失败的错误处理
       - TerminalEvents.jsx：统一终端信息和事件记录的错误处理
    4. **完善所有页面的统一错误处理**：
       - Register.jsx：使用parseErrorMessage改进注册错误处理
       - Login.jsx：使用parseErrorMessage改进登录错误处理
       - SystemSettings.jsx：统一系统设置加载和保存的错误处理
       - DictionaryListView.jsx：改进字典列表加载错误处理
       - RoleListView.jsx：统一角色创建和删除的错误处理
       - LinePriceEditView.jsx：改进线路票价保存错误处理
       - UserCreateDialog.jsx：统一用户创建和数据加载错误处理
  - 技术细节：
    - 错误解析支持字符串、对象、验证错误等多种格式
    - HTTP状态码映射：401登录过期、403权限不足、404资源不存在、5xx服务器错误
    - 网络错误检测：超时、连接失败等情况
    - 保持向后兼容，不影响现有正常工作的错误处理
  - 影响：大幅提升用户体验，避免页面崩溃，提供友好的错误信息和操作反馈
  - 覆盖范围：已完成所有主要页面的统一错误处理，包括：
    - 认证页面：Login、Register
    - 管理页面：UserList、RoleList、MerchantList、DictionaryList
    - 业务页面：TerminalList、FilePublish、MessageType、LinePriceEdit
    - 系统页面：SystemSettings、Dashboard
    - 对话框组件：UserCreateDialog、DictionaryFormDialog等
  - 技术标准：所有页面现在都使用parseErrorMessage+enqueueSnackbar的统一模式

- **修复文件发布页面滚动条问题**：
  - 问题现象：文件发布时，线路选择和终端选择对话框中的列表超出页面范围时没有滚动条，无法查看完整内容
  - 根本原因：
    1. 对话框高度设置不合理，没有考虑不同屏幕尺寸
    2. TableContainer的maxHeight设置为固定值，在小屏幕上可能超出对话框范围
    3. 缺少flex布局来合理分配空间
    4. 滚动条样式不够明显
  - 解决方案：
    1. **优化对话框布局**：
       - 线路选择对话框：设置高度为80vh，最大600px
       - 终端选择对话框：设置高度为85vh，最大700px（因为内容更多）
       - 使用flex布局确保内容区域能正确分配空间
    2. **改进TableContainer滚动**：
       - 移除固定的maxHeight限制，改用flex: 1自适应高度
       - 添加自定义滚动条样式，提高可见性
       - 确保overflow: auto正常工作
    3. **优化布局结构**：
       - DialogContent使用flex布局，防止内容溢出
       - 筛选条件区域设置flexShrink: 0，确保不被压缩
       - 表格区域使用flex: 1，占用剩余空间
  - 技术细节：
    - 使用PaperProps自定义对话框容器样式
