  ...
  static renderHeader(
    ...
    if (config.title) {
      ...
      if (config.subtitle) {
        ...
        } else if (typeof config.subtitle === 'object' && config.subtitle.text) {
          subtitleText = config.subtitle.text;
        } else {
          subtitleText = String(config.subtitle);
        }

        subtitleElement.textContent = subtitleText;

        // 应用副标题样式(如果有)
        if (typeof config.subtitle === 'object' && config.subtitle.style) {
          Object.entries(config.subtitle.style).forEach(([prop, value]) => {
            subtitleElement.style[prop as any] = value as string;
          });
        }
